...
import '../services/io_wrapper.dart';
...
class DownloadProvider with ChangeNotifier {
  ...
  void _stopForegroundIfNoActive() {
    if (PlatformUtils.isAndroid && _activeCount <= 1) {
  ...
  Future<void> _startDownload(DownloadItem item) async {
    ...
    // Web handles storage differently, skip direct file IO checks for now
    ...
    try {
      ...
      if (!PlatformUtils.isWeb && existingBytes > 0 && total > 0 && existingBytes >= total) {
      ...
      if (PlatformUtils.isWeb) {
        ...
        await _downloadWeb(item, cancelToken);
      } else {
  ...
  Future<void> _downloadWeb(DownloadItem item, CancelToken cancelToken) async {
    final dio = DioClient().dio;
    await dio.download(
      item.url,
      item.savePath,
